...
MCQController.getVerticalTemplate = function () {
  return "<div class='qc-option-container'> \
   <% if(question.config.layout == 'Vertical' || (question.config.layout == undefined)){ %> \
     <% _.each(question.data.options, function(val,key,index) { %> \
      ...
      <div class='qc-opt'>\
       <%if(_.isEmpty(val.audio)){%> \
         <img class='qc-vertical-audio-disable' src=<%=MCQController.pluginInstance.getDefaultAsset(MCQController.pluginInstance._defaultAudioIcon) %>>\
        <%}else{%> \
           <img class='qc-vertical-audio' onclick=MCQController.pluginInstance.playAudio('<%= val.audio %>')  src=<%=MCQController.pluginInstance.getDefaultAsset(MCQController.pluginInstance._defaultAudioIcon) %>>\
           <%}%>\
      </div>\
      <% }else if(MCQController.isAudioIcon&& !MCQController.isImageIcon){ %> \
         <div class='qc-opt'>\
       <%if(_.isEmpty(val.audio)){%> \
         <img class='qc-vertical-audio-txt-disable' src=<%=MCQController.pluginInstance.getDefaultAsset(MCQController.pluginInstance._defaultAudioIcon) %>>\
        <%}else{%> \
           <img class='qc-vertical-audio-txt' onclick=MCQController.pluginInstance.playAudio('<%= val.audio %>')  src=<%=MCQController.pluginInstance.getDefaultAsset(MCQController.pluginInstance._defaultAudioIcon) %>>\
           <%}%>\
      </div>\
        <%}%>\
